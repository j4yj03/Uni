...
f = @(x) Q(sqrt(2*ebN0lin*x)).*(1-exp(-x)).^(Nr-1).*exp(-x);
P_RAY_QPSK_SDC = Nr.* integral(f, 0, Inf, 'ArrayValued', true);

%Ray MRC
Q = @(y) 0.5 * erfc(y/sqrt(2));
f = @(x) Q(sqrt(2*ebN0lin*x)).*x^(Nr-1).* exp(-x);
P_RAY_QPSK_MRC = (1/gamma(Nr)).* integral(f, 0, Inf, 'ArrayValued', true);
...
% =========================================================================
%% simulations loop...
sprintf('K = %d, Anzahl der Antennen %d..', K, Nr)  % Ausgabe der aktuellen SNR    
for i=1:length(esN0dB)
    ...
    while(totalFehler < nMinErr && nProcessedBits < nMaxBits)
        ...
        compensatedSymbols=fadingChannel(i, mappedSymbols, esN0dB(i), K, Nr, combMethod);
...
% =========================================================================
% =========================================================================
function y=fadingChannel(i, mappedSymbols, SNR, K, Nr, combMethod)
...
% Ausgabeparameter: es werden die kompensierten Symbole ausgegeben (y)
    ...
    y = antennaCombining(receivedsymbols, h, combMethod);
end
%==========================================================================
...
% Ausgabeparameter (y): Eingangssignal mit additiven Rauschen
    ...
    N0 = (1/10^(snrdb/10)); %Rauschleistung
...
% ausgegeben
    ...
    H_LOS = sqrt(mean2) * (ones(Nr, nSamp)).* exp(1j*2*pi*randn(1, nSamp));  % h-Koefizient LOS

    y = (H_LOS + H_NLOS); % normal verteilte Kanalkoefizienten
    
    %Mean = mean(mean(abs(y).^2))
